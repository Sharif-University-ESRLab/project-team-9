...
repeat_weekly_weeks = 10  # we will repeat for 10 weeks after asked from NodeMCU, if repeat weekly was checked.
# this will be repeated every time NodeMCU asks for an update.


# these are the voice commands variations, as captured per each command.
...
def manualcontrol(request):  # this is for the section 'کنترل از راه دور'
...
@ensure_csrf_cookie
def schedule(request):  # for loading the schedule webpage
    template = loader.get_template('schedule.html')
    return HttpResponse(template.render())


def getupdate(request):  # for the NodemCU to ask for the lamps' status
...
def change_lamp(id, state):
    # a method for setting the status of a lamp, used by other methods of views and not directly
    lamp = Lamps.objects.get(id=id)
    lamp.status = state
    lamp.save()


def turnon(request, id):  # turning on a lamp
    change_lamp(id, True)
    return HttpResponseRedirect(reverse('manualcontrol'))


def turnoff(request, id):  # turning off a lamp
    change_lamp(id, False)
    return HttpResponseRedirect(reverse('manualcontrol'))


def report(request):
    # getting the status of lamps FROM the NodeMCU module, in case the user physically makes any changes
...
def schedule_update(request):  # adds a new column to our schdule (an instance of 'Schedule' model in database)
...
def get_schedule(reuqest):
    # reports the schedule for the upcoming settled events, in a format decipherable for the module.
    # in particular, we report the milliseconds left till every event.
    ...
    for change in schedule:
        ...
        status += lamp_no + lamp_status + " " + str(int(difference.total_seconds() * 1000 - 4500 * 3600)) + " "
        if change['repeat_weekly']:
            for no_week in range(repeat_weekly_weeks - 1):
                status += lamp_no + lamp_status + " " + str(
                    int((difference.total_seconds() - 4500 * 3600 + 7 * 24 * 3600 * (no_week + 1)) * 1000)) + " "

    return HttpResponse(status)


def viewschedule(request):  # directs to the HTML page of viewing the schedule
...
def delete_schedule(request, id):  # in case the user wants to delete an event
    sch = Schedule.objects.get(id=id)
    sch.delete()
    return HttpResponseRedirect(reverse('viewschedule'))


def process_voice(request):  # processes the many voice commands uttered by the user, with variations taken into account
